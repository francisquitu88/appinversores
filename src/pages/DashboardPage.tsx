...
function displayCalendarDate(value: string | null) { return value ? calendarDateFormatter.format(new Date(`${value.slice(0, 10)}T12:00:00Z`)) : '-' }
...
export function DashboardPage({ session, onNavigate }: { session: Session; onNavigate: (path: string) => void }) {
  ...
  const summaries = useMemo(() => {
    ...
    stocks.forEach((stock) => result.set(stock.ticker, { ticker: stock.ticker, totalNews: 0, totalFilings: 0, totalRatings: 0, totalInsiders: 0, latestNews: null, latestFiling: null, latestRating: null, latestInsider: null }))
    items.forEach((item) => {
      if (!item.ticker || !result.has(item.ticker) || item.item_type === 'post') return
      const summary = result.get(item.ticker)!
      if (isSec(item)) {
        summary.totalFilings += 1
        if (!summary.latestFiling || (item.published_at && item.published_at > summary.latestFiling)) summary.latestFiling = item.published_at
      } else {
        summary.totalNews += 1
        if (!summary.latestNews || effectiveDate(item) > new Date(summary.latestNews).getTime()) summary.latestNews = item.published_at ?? item.scraped_at
      }
    })
