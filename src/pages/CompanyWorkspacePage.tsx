...
function lastUpdated(data: CompanyData) { const dates = [...data.items.map((item) => { const calendar = item.metadata.source === 'SEC' || typeof item.metadata.accessionNumber === 'string'; const value = calendar ? calendarDateTimestamp(item.published_at ?? item.scraped_at) : new Date(item.published_at ?? item.scraped_at).getTime(); return { value, calendar } }), ...data.ratings.map((item) => ({ value: calendarDateTimestamp(item.rating_date), calendar: true })), ...data.insiderTrades.map((item) => ({ value: calendarDateTimestamp(item.transaction_date), calendar: true }))]; const latest = dates.reduce((current, candidate) => candidate.value > current.value ? candidate : current, { value: 0, calendar: true }); if (!latest.value) return 'No data'; return new Intl.DateTimeFormat('en-US', { dateStyle: 'medium', timeZone: latest.calendar ? 'UTC' : 'America/New_York' }).format(new Date(latest.value)) }
